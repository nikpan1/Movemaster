...
class PoseLandmarkExtractor:
    def __init__(self, min_detection_confidence: float = 0.7, min_tracking_confidence: float = 0.7, window_size=90):
        """
        ...
        """
        self.mp_pose = mp.solutions.pose.Pose(min_detection_confidence=min_detection_confidence,
    ...
    def convert_history_to_tensor(landmark_history: deque, device) -> torch.Tensor:
        ...
        tensor = torch.from_numpy(history_array).permute(2, 0, 1).unsqueeze(0).float().to(
            device)  # Shape: (1, 2, 150, 18)
    ...
    def mediapipe_to_openpose(self, mediapipe_landmarks: np.ndarray) -> np.ndarray:
        ...
        mapping = {0: 0, 1: (12, 11),  # Average of points 12 and 11
            2: 12, 3: 14, 4: 16, 5: 11, 6: 13, 7: 15, 8: 24, 9: 26, 10: 28, 11: 23, 12: 25, 13: 27, 14: 5, 15: 2, 16: 8,
            17: 7}

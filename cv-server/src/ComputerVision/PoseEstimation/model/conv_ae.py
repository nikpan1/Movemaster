...
class ConvAutoencoder(nn.Module):
    def __init__(self, layers, input_size, grouped, dim, input_ch=2, kernel_size=5, kernel_stride=3,
        ...
        self.conv1 = conv_f(input_ch, 8//ch_mult, kernel_size=kernel_size, stride=kernel_stride,
                            padding=padding, groups=grouped[0])
        if self.layers > 1:
            self.conv2 = conv_f(8//ch_mult, 12//ch_mult, kernel_size=kernel_size, stride=kernel_stride,
                                padding=padding, groups=grouped[1])
        if self.layers > 2:
            self.conv3 = conv_f(12//ch_mult, 20//ch_mult, kernel_size=kernel_size, stride=kernel_stride,
